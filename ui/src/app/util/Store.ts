...
export class StoreCollection<T> extends Store<T[]> {
  ...
  updateById(item: T, filter: (T) => boolean = it => it.id === item['id']): T {
    if (this.data.value.find(filter)) {
      this.data.next(this.data.value.map(it => filter(it) ? item : it));
      return item;
    } else {
      return null;
    }
  ...
  removeById(item: T, filter: (T) => boolean = it => it.id === item['id']): T {
    if (this.data.value.find(filter)) {
      this.data.next(this.data.value.filter(it => !filter(it) ));
      return item;
    } else {
      return null;
    }
